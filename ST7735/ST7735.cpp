...
void ST7735::fillRectBar(int16_t x, int16_t y, int16_t w, int16_t h, short array[10][50]) {
  ...
  uint8_t hi = array[0][0] >> 8, lo = array[0][0];
...
void ST7735::drawOneChar(int16_t x, int16_t y, const short *num)
{
	int CHAR_ROW = 8;
	int CHAR_COL = 5;
	int h = CHAR_ROW;
...
// x = w = rows, y = h = cols
  for(x=0;x<CHAR_ROW; x++) {
    for(y=0; y<CHAR_COL; y++) {
		hi = *(num + x*CHAR_COL + y) >> 8;
		lo = *(num + x*CHAR_COL + y);
